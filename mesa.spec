...
Source0:        https://codeload.github.com/nicOS-v/mesa-img/zip/refs/heads/21.3.8-ddk1.17
...
Conflicts:      mesa-dri-filesystem > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libGL > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libGL-devel > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libEGL > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libEGL-devel > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-dri-drivers > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-omx-drivers > %{?epoch:%{epoch}:}%{version}-%{release}
Requires:       %{name}-filesystem%{?_isa} = %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-vdpau-drivers > %{?epoch:%{epoch}:}%{version}-%{release}
Requires:       %{name}-filesystem%{?_isa} = %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libOSMesa > %{?epoch:%{epoch}:}%{version}-%{release}
Requires:       %{name}-libglapi%{?_isa} = %{?epoch:%{epoch}:}%{version}-%{release}
Provides:       libOSMesa
...
Conflicts:      %{name}-libOSMesa-devel > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libgbm > %{?epoch:%{epoch}:}%{version}-%{release}
Provides:       libgbm
...
Conflicts:      %{name}-libgbm-devel > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libxatracker > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libxatracker-devel > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libglapi > %{?epoch:%{epoch}:}%{version}-%{release}
Provides:       libglapi
...
Conflicts:      %{name}-libOpenCL > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-libOpenCL-devel > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-vulkan-drivers > %{?epoch:%{epoch}:}%{version}-%{release}
...
Conflicts:      %{name}-vulkan-devel > %{?epoch:%{epoch}:}%{version}-%{release}
...
%endif
# Temp solution
# Because of 0013-wayland-drm-install-wayland-drm.xml-to-the-configure.patch
# maybe we should have this in it's own package?
%{_datadir}/mesa/wayland-drm.xml
%{_datadir}/pkgconfig/wayland-drm.pc
%endif
...
%endif
# For disable swrast
%exclude %{_libdir}/libvulkan_lvp.so
%exclude %{_datadir}/vulkan/icd.d/lvp_icd.*.json
# For pvr
%{_libdir}/dri/pvr_dri.so
/usr/lib64/libpvr_mesa_wsi.so
...
%changelog
* Thu Feb 8 2024 kiritakekumi <pujingyu@iscas.ac.cn> - 21.3.8-1
- update 21.3.8
- Add pvr support

* Mon Dec 11 2023 misaka00251 <liuxin@iscas.ac.cn> - 21.2.1-3
- Add Conflict to prevent newer version of mesa

* Wed Nov 22 2023 misaka00251 <liuxin@iscas.ac.cn> - 21.2.1-2
- Add pvr support

* Fri Sep 22 2023 misaka00251 <liuxin@iscas.ac.cn> - 21.2.1-1
- Downgrade to mesa-21.2.1
